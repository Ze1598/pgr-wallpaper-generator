...
def main(
    ...
    ignore_bg_image = (background_art == None)
    ignore_fg_image = (foreground_art == None)
    ...
    if ignore_fg_image != True:
        ...
        # shadow = shadow.filter(ImageFilter.BoxBlur(10))
        wip_img.paste(shadow, shadow_coords, mask=fg_art)
        
        # Add the art as its own blurry shadow
        # fg_art_shadow = fg_art.copy()
        # alpha = fg_art_shadow.getchannel("A")
        # alpha = ImageEnhance.Brightness(alpha).enhance(0.5)
        # fg_art_shadow.putalpha(alpha)
        # fg_art_shadow = fg_art_shadow.filter(ImageFilter.BoxBlur(10))
        # wip_img.paste(fg_art_shadow, shadow_coords, mask=fg_art_shadow)
