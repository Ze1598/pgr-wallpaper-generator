...
def prepare_loaded_bg_art(res, img_dimensions, alpha):
    ...
    # Now resize to appropriate dimensions        
    art_dims = art.size
    resize_factor = 0.4
    resized_width = int(art_dims[0] * resize_factor)
    resized_height = int(art_dims[1] * resize_factor)
    art = art.resize((resized_width, resized_height))
...
def create_and_paste_footer(img: Image.Image, operator_color: str) -> None:
    ...
    footer_img = change_alpha(footer_img, 0.5)
